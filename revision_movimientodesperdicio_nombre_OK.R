revision_mov_desperdicio_nombre <- function (inventario_nombre, tiendas, fechas, inventario = "In Store Movement")
...
#
    ...
    desperdicioSM1_producto <<- ddply (desperdicioSM1, "product_id.name", summarise,
...
#   
    ...
    desperdicioSM3_producto <<- ddply (desperdicioSM3, "product_id.name", summarise,
                                       subtotal = sum(product_id.standard_price*product_qty),
                                       cantidad = sum(product_qty))
#
    ...
    desperdicioSM5_producto <<- ddply (desperdicioSM5, "product_id.name", summarise,
                                       subtotal = sum(product_id.standard_price*product_qty),
                                       cantidad = sum(product_qty))
    ...
    write.csv(desperdicioSM1_producto, file = paste("SM1_", inventario_nombre, tiendas, fechas, ".csv", sep = ""))
    write.csv(desperdicioSM3_producto, file = paste("SM3_", inventario_nombre, tiendas, fechas, ".csv", sep = ""))
    write.csv(desperdicioSM5_producto, file = paste("SM5_", inventario_nombre, tiendas, fechas, ".csv", sep = ""))
